...
def realtime_query():
    ...
    filter_list = {}
    for d in dirs:
        dir = os.path.join(os.environ.get('HOME', '~'), 'project/runs', d)
...
def get_all_experiment(sub):
    directory = os.path.join(os.environ['HOME'], 'project/runs', sub)
    lists = sorted(os.listdir(directory))
    filter_list = []
    for e in lists:
        if 'copy' not in e:
...
def get_result_array():
    ...
    full_name = os.path.join(os.environ['HOME'], 'project/runs', file_name)
...
@app.route('/gpu_process.html')
def gpu_process():
    return render_template('gpu_process.html')


@app.route('/operate_dir', methods=['POST'])
def operate_dir():
    ...
    return json.dumps({'info': "error"}, ensure_ascii=False)